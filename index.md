...
![The figure illustrates the process of CLI (left) and unlearning (right). Left: the server computes the difference of each layer between the models uploaded by each client and the distributed one, identifying is constructed for each key layer, which is then independently trained on the remaining data.](/static/image/method.jpg)
